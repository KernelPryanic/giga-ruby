...
function checkPage(element) {
    var attrib = element.getAttribute("page");
    var used = getCookie("page");
    if (!used) {
        used = "main"
    }
    return attrib == null || attrib == used || attrib == "any";
}

function update(doc) {
    applyLanguage(doc);
    applyPage(doc);
    HideScrollBar("container2");
}
...
function HideScrollBar(containerName) {
    var containers = document.getElementsByClassName(containerName);

    for (i in containers) {
        if (containers[i].style) {
            containers[i].style.paddingRight = containers[i].offsetWidth - containers[i].clientWidth + "px";
        }
    }
}
